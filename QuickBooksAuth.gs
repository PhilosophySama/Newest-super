 ...
 * - QBO_REALM_ID: Your QuickBooks company ID (set automatically after authorization)
...
function authorize() {
  try {
    ...
    if (service.hasAccess()) {
      SpreadsheetApp.getActive().toast("QuickBooks already authorized!", "✅ Already Connected", 3);
      return;
    }
      ...
      <html>
        <head>
          <style>
            ...
            .debug { margin-top: 30px; font-size: 11px; color: #999; word-break: break-all; }
          </style>
        </head>
        <body>
          <h3>Connect to QuickBooks</h3>
          <p>Click below to authorize this spreadsheet to access your QuickBooks company.</p>
          <a class="btn" href="${authorizationUrl}" target="_blank">Authorize QuickBooks</a>
          <div class="debug">
    ...
    <html>
      ...
      <body>
        ...
        <ol>
          ...
          <li>Set all required Script Properties (Project Settings → Script Properties):
            <ul>
              <li>QBO_CLIENT_ID</li>
...
/**
 * Menu-triggered setup instructions popup (RECONSTRUCTED — see note above code block)
 */
function showQuickBooksSetup_() {
  const ui = SpreadsheetApp.getUi();
  ui.alert(
    'QuickBooks Setup — Order of Operations',
    '1. Set Script Properties: QBO_CLIENT_ID, QBO_CLIENT_SECRET, QBO_ENVIRONMENT\n' +
    '2. Deploy this project as a Web App (Deploy → New Deployment)\n' +
    '3. Run "Get Web App URL" from this menu, copy the URL\n' +
    '4. Paste that URL into Script Properties as QBO_REDIRECT_URI\n' +
    '5. Paste that SAME URL into your Intuit Developer app\'s Redirect URIs\n' +
    '6. Run "Authorize QuickBooks" from this menu\n' +
    '7. Run "Test Connection" to confirm\n\n' +
    'Once connected, use "Send Estimate (Current Row)" from the Leads sheet.',
    ui.ButtonSet.OK
  );
}