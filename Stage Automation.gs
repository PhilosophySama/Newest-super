...
/*** FEATURE: Format QB URL (P) as "QB" hyperlink if https, else plain text - Works on ALL sheets ***/
function handleQbUrlChange_(sheet, row, _val) {
  const S = MOVE_CONFIG;
  const target = sheet.getRange(row, S.COLS.QB_URL);

  const rawVal = String(target.getValue() || '').trim();
  if (!rawVal) return;

  if (rawVal.toLowerCase().startsWith('https')) {
    // It's a URL → format as "QB" hyperlink
    ...
    // Not a URL → paste as plain text, no wrap
    target.setValue(rawVal);
    target.setWrap(false);
    if (S.ENABLE_LOGGING) m_logOperation_('QB cell set as plain text (no wrap)', {value: rawVal.substring(0, 50), row, sheet: sheet.getName()});
  }
}
...
function handleStageChange_(e, sheet, row, newStage) {
  ...
  // Special case: "QB Prompt" - copies Re-cover pricing template into col P (Leads only)
  if (isLeads && m_stageMatches_(stage, S.STAGE_MAPPINGS.qb_prompt)) {
    m_handleQbPrompt_(ss, sheet, row);
  ...
  } else if (isAwarded && m_stageMatches_(stage, S.STAGE_MAPPINGS.negotiating)) {
    dest = sheets.fu;
    reason = 'Negotiating → moved to F/U';
      ...
      if (S.ENABLE_LOGGING) {
        m_logOperation_('Row moved', {
          customer: customerName, stage, from: sheet.getName(), to: dest.getName(), originalRow: row
            ...
            try { al_logActivity_(dest.getName(), 'Automation', customerName, displayName, 'Move', sheet.getName() + ' → ' + dest.getName(), '', null, '', reason); } catch (_) {}
...
function m_createPrintPacket_(sheet, row) {
  ...
  try {
    ...
    if (imageAttachments.length === 0) {
      logCell.setValue('⚠️ No image attachments found in email — will use Drive photos only.');
      SpreadsheetApp.flush();
    }
    ...
    const driveImages = [];
    try {
      const driveFiles = photosFolder.getFiles();
      while (driveFiles.hasNext() && driveImages.length < 3) {
      ...
      if (driveImages.length > 0) {
        logCell.setValue(`🖼️ Found ${driveImages.length} Drive image(s) to include...`);
        SpreadsheetApp.flush();
      }
    } catch (driveImgErr) {
      if (S.ENABLE_LOGGING) {
        m_logOperation_('Drive image fetch error', { error: driveImgErr.message });
      }
    }
    ...
    // ===== PHOTO PAGES (email attachments + Drive folder images) =====
    const totalPhotoCount = imageAttachments.length + driveImages.length;
    logCell.setValue(`📸 Adding ${imageAttachments.length} email photo(s) + ${driveImages.length} Drive photo(s)...`);
    SpreadsheetApp.flush();
    ...
    // ===== DRIVE FOLDER PHOTO PAGES =====
    for (let i = 0; i < driveImages.length; i++) {
      body.appendPageBreak();
      
      try {
        const image = body.appendImage(driveImages[i]);
      ...
      } catch (driveImgAppendErr) {
        body.appendParagraph(`Error loading Drive image ${i + 1}: ${driveImgAppendErr.message}`)
          .setAlignment(DocumentApp.HorizontalAlignment.CENTER);
      }
    }
...
/*** FEATURE: Copy Re-cover pricing prompt into col P when stage = "QB Prompt" ***/
function m_handleQbPrompt_(ss, sheet, row) {
  const S = MOVE_CONFIG;
  const target = sheet.getRange(row, S.COLS.QB_URL); // col P

  // If P already displays "QB", do not interfere
  const currentDisplay = target.getDisplayValue().trim();
  if (currentDisplay === 'QB') {
    if (S.ENABLE_LOGGING) m_logOperation_('QB Prompt skipped - P already has QB', {row});
    return;
  }
  ...
  try {
    ...
    // Get Re-cover sheet
    const recoverSheet = ss.getSheetByName(S.SHEETS.RECOVER);
    if (!recoverSheet) {
      SpreadsheetApp.getActive().toast('Re-cover sheet not found!', 'QB Prompt Error', 5);
      return;
    }

    // Write customer name into Re-cover K2 (dropdown lookup input cell)
    // Must bypass data validation temporarily since setValue() respects dropdown restrictions
    const k2Range = recoverSheet.getRange('K2');
    k2Range.clearDataValidations();
    k2Range.setValue(customerName);
    // Always restore a fresh whole-column validation rule so #REF! can never persist
    const freshValidation = SpreadsheetApp.newDataValidation()
      .requireValueInRange(
        SpreadsheetApp.getActive().getSheetByName(S.SHEETS.LEADS).getRange('E:E'),
        true
      )
      .setAllowInvalid(true)
      .build();
    k2Range.setDataValidation(freshValidation);
    SpreadsheetApp.flush(); // Allow formulas to recalculate

    // Pick range based on job type
    const isRecover = jobType.toLowerCase() === 're-cover';
    const promptRange = isRecover
      ? recoverSheet.getRange('A37:D57')
      : recoverSheet.getRange('M37:N66');

    // Read and flatten: columns merged (no separator), rows joined with newlines
    const values = promptRange.getValues();
    const lines = values
      .map(rowArr => rowArr.map(cell => String(cell || '').trim()).join(''))
      .filter(line => line.length > 0); // drop fully empty rows

    const promptText = lines.join('\n');

    // Paste into col P as plain text, wrap off
    target.setValue(promptText);
    target.setWrap(false);
    ...
    SpreadsheetApp.getActive().toast('QB Prompt error: ' + err.message, 'Error', 5);
    if (S.ENABLE_LOGGING) m_logOperation_('QB Prompt error', {row, error: err.message});
  }
}
